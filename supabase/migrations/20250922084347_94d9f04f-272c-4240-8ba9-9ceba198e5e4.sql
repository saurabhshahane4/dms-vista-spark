...
-- Create workflow audit table for better tracking
...
-- Create RLS policies for workflow_audit_log
CREATE POLICY "Users can view their workflow logs" ON public.workflow_audit_log
  FOR SELECT USING (user_id = auth.uid());

CREATE POLICY "Admins can view all workflow logs" ON public.workflow_audit_log
  FOR SELECT USING (public.has_role(auth.uid(), 'admin'));
...
BEGIN
  ...
  FOR workflow_trigger IN 
    SELECT * FROM public.document_workflow_triggers dwt
  ...
  LOOP
    ...
    INSERT INTO public.notifications (
      ...
      COALESCE(NEW.user_id, OLD.user_id),
      'Workflow Triggered',
      'Workflow "' || workflow_trigger.name || '" has been triggered by document ' || TG_ARGV[0],
      'info',
      jsonb_build_object(
